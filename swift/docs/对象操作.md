...
- 分块上传对象： 初始化分块上传，  上传分块块， 完成所有分块块上传。
- 分块续传：查询已上传的分块块， 上传分块块，完成所有分块块上传。
- 删除已上传分块块。
...
查询指定 uploadId 已上传的分块块（List Parts）。
...
cos iOS SDK 中舍弃一个分块上传且删除已上传的分块块请求的方法具体步骤如下：
...
| --------- | ------------------------------------------------------------ | ---------- | ---- |
| bucket    | 存储桶名，可在 [COS 控制台 ](https://console.cloud.tencent.com/cos5/bucket) 查看，格式为&lt;BucketName-APPID&gt; ，例如 examplebucket-1250000000 | NSString * | 是   |
| object    | 对象键（Key）是对象在存储桶中的唯一标识。<br>例如，在对象的访问域名`examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg` 中，对象键为 doc/pic.jpg，更详细的描述可以参见 [对象描述](https://cloud.tencent.com/document/product/436/13324)。 | NSString * | 是   |
